...
router.post('/', authMiddleware, requireCustomer, async (req, res) => {
  ...
  if (existing) {
    ...
    const updated = await cardRepo.update(existing.id, {
      ...
      business_complement: req.body.business_complement !== undefined ? String(req.body.business_complement).substring(0, 180) : existing.business_complement,
  ...
  const card = await cardRepo.insert({
    ...
    business_complement: req.body.business_complement ? String(req.body.business_complement).substring(0, 180) : null,
...
router.put('/:id', authMiddleware, requireCustomer, async (req, res) => {
  ...
  const updated = await cardRepo.update(card.id, {
    ...
    business_complement: req.body.business_complement !== undefined ? String(req.body.business_complement).substring(0, 180) : card.business_complement,
